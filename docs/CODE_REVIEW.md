...
| DES-01 | P1 | **FIXED (host-verified)** | YR2/YE1 *intermediates* replaced by a root bundle — ISRG Root X1 + ISRG Root X2 + Amazon Root CA 1 — applied as one `ROOT_CA_BUNDLE` for every host. Verified against all three LIVE chains with `openssl verify -CAfile <bundle> -untrusted <chain> <leaf>`. This also exposed a defect this review did not list (see below). **Not yet exercised on hardware** — no port attached. |
