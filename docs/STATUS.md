...
| Verse-first layout (2026-09-19) | ✅ **The verse owns the panel.** The weather column (70 px, 24% of the width) and the mode title are gone; the header band now identifies the content (citation, or word + respelling) at 7pt with the date right, the verse auto-sizes to **8pt** for short/medium content (7pt medium-length, 5.5pt longest — all above the old 6pt ceiling for typical content, measured on the device font path), and a footer row carries status/warnings bottom-left and the weather as text bottom-right. Driven by readability: the owner reads this panel at 55 (see LESSONS §53) |
