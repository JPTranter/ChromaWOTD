...
| Stored config can be wiped by the NVS stack | 🔧 **FIX IMPLEMENTED, awaiting bench verification (LESSONS §48).** The configuration now lives in its own 16 KB `nvs_cfg` partition (`firmware/partitions.csv` + `board_build.partitions` + a label on all three `Preferences::begin()` calls), out of reach of the core's erase of the shared 20 KB `nvs` partition. Verified locally: the built table decodes with `nvs` still FIRST (the core finds its erase target with a NULL label), `nvs`/`otadata`/`app0` offsets are unchanged so `merge_firmware.py` and the app-only-flash path still hold, and `env:nvsprobe_legacy` reproduces the previous table byte-for-byte. `env:nvsprobe` / `env:nvsprobe_legacy` are the destructive A/B proof and still need a bench run |
