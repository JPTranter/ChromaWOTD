...
|---|---|
| Repo scaffold | ✅ done |
| Firmware builds | ✅ `verify_all.py` **ALL GREEN on a clean tree** (2026-09-22): `pio run -e s3` Flash **30.9%** (1,031,865 / 3,342,336 B), RAM **24.6%** (80,548 / 327,680 B); only library-side warnings. The 30.8% / 23.8% figures before this session predate the WOTD body buffers added below |
...
| Verse font auto-size | ✅ Ladder extracted to `cc_pickVerseFont()`/`cc_verseFontSize()`; block geometry lives once in `verse_display.h` (`kVerseMaxW`/`kVerseMaxH` = **288×93**). The ladder is **10 / 9 / 8 / 7 / 6 / 5.5 / 5pt** (widened from 6/5.5/5 with the verse-first layout, then raised to 10pt so short content can fill the panel — LESSONS §53/§55). Measured on the device font path: short verses and Word-of-the-Day definitions get **10pt**, the 153-char fixture verse 8pt, 191 chars 7pt, 316 chars 5.5pt, and a paragraph-length WOTD body 5pt |
| Verse block height (2026-09-22) | ✅ **FIXED (LESSONS §68).** The box stopped at y=104 (82px) and reserved a fixed `8*size` px inset, so the 5pt rung drew 6 of the 7 lines that fitted — reported from the panel as "should allow for another line, as there is space" (18px, a whole line, sat unused above the footer). The box is now the space between the two fixed rows (22 → the footer row's top at 115 = **93px**) and `cc_lineCapacity(maxH, lineHeight) = maxH / lineHeight` holds only WHOLE lines, with no pixel inset to get wrong; both draw loops stop on the same capacity the truncation decision uses. The largest rung is unaffected (93/26 = 3 lines; 4 would have reached y=126, into the footer), and the 4 canonical ledger renders are byte-identical. The portal's wrapped error text keeps its 9 lines (box 84 → 90) |
