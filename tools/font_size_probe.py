...
def ensure_built():
    if not os.path.exists(os.path.join(BUILD_DIR, "CMakeCache.txt")):
        ...
        subprocess.run(["cmake", "-S", TEST_DIR, "-B", BUILD_DIR, "-G", "Ninja",
                        "-DCHROMAWOTD_DEVICE_FONTS=ON"], cwd=ROOT, check=True)
    subprocess.run(["cmake", "--build", BUILD_DIR], cwd=ROOT, check=True,
...
def main():
    ...
    if args.live:
        ...
        cmd.append(mode)
        print(f"content mode: {'Verse of the Day' if hour < 12 else 'Word of the Day'} (local hour {hour})")
